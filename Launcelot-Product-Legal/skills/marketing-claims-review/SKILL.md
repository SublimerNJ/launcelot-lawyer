...
# /launcelot-product-legal:marketing-claims-review

This skill is the extraction-and-mapping core of Launcelot-Product-Legal. It does not decide whether a phrase is criminal — that requires statute and case-law verification, which is delegated to `launcelot-lawyer`. This skill produces a structured list of every line that *could* trigger criminal or regulatory exposure, mapped to the relevant 죄목 or 규정, with a draft risk band and a suggested fix.

## Run order

1. Load `~/.claude/plugins/config/launcelot-product-legal/CLAUDE.md`. If the file does not exist or contains `[PLACEHOLDER]` markers, stop and tell the user to run `/launcelot-product-legal:cold-start-interview` first.
...
- If the script contains zero pull-bucket hits, emit:
  ...
  없다는 결론이 아닙니다. 확실한 결론을 원하면 /launcelot-product-legal:launch-review로
