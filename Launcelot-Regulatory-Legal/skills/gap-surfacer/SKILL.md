...
# /launcelot-regulatory-legal:gap-surfacer
...
1. Load `~/.claude/plugins/config/launcelot-regulatory-legal/CLAUDE.md` — `## Gap tracker policy`.
2. Load `~/.claude/plugins/config/launcelot-regulatory-legal/gap-tracker.yaml`. Create if absent.
...
1. Hard-rule hits first (any rule from `Launcelot-Product-Legal`의 `## Hard rules`에 매칭되는 갭).
2. Tier 1 변경에서 유래한 갭.
...
- `revision_ref`: the launch-review revision in Launcelot-Product-Legal that recorded the fix (slug-B + revision number). If Launcelot-Product-Legal is not installed, accept a free-text proof but record the absence.
...
Close as `비공개결정`. Means the underlying script was taken down or set unlisted to resolve the gap. Required: `unpublish_action` (delete / unlist / set-private) and `acted_at`. The skill checks `_index.yaml`'s `current_recommendation` for that slug-B from Launcelot-Product-Legal; if Launcelot-Product-Legal is installed and the slug-B's status does not match (e.g., still `게시함`), flag a mismatch and refuse to mark `비공개결정` until the user reconciles.
...
- 권장 후속:
  ...
  - `/launcelot-regulatory-legal:policy-redraft <pattern-id>`로 공통 수정 방향을 한 번에 받은 뒤,
    `/launcelot-regulatory-legal:policy-diff <slot_a_id> library`를 다시 돌려 누락을 확인하라.
  - Launcelot-Product-Legal가 설치되어 있다면 `## Hard rules`에 이 패턴을 등록할지 고려하라.
```
...
- Launcelot-Product-Legal의 `launch-review` 결정 로그를 변경하지 않는다. 두 트래커는 분리되어 있고 cross-reference만 한다.
