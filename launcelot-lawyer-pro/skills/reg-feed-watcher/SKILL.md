...
1. Load `~/.claude/plugins/config/launcelot-lawyer-pro/CLAUDE.md` — `## Watchlist`, `## Materiality threshold`, `## Cadence`, `## Integrations`.
...
Path: from `## Cadence` config; default `~/.claude/plugins/config/launcelot-lawyer-pro/digests/<YYYY-MM-DD>.md`.
...
- 사용자가 Tier 1에 "변협 광고규정 본문 개정"을 넣었는데 watchlist에 변협이 없다 → 디제스트 상단에 한 번만:
  > ⚠ 커버리지 누락: Tier 1 정의에 "변협 광고규정"이 포함되어 있으나 watchlist에 koreanbar.or.kr이 없다. `/launcelot-lawyer-pro:customize --field "Watchlist"`로 추가하거나, 본 cadence 구간에서 일회성으로 건너뛸 수 있다.
